...
public class ArsenalCommands extends BaseCommand {
    ...
    @Subcommand("bastion give")
    public void onBastionGive(Player player, String type) {
        BastionType bastionType = BastionType.getBastionType(type);
        ItemStack itemStack = bastionType.getItemRepresentation();
    ...
    public void onStructureBlueprintView(Player player, String blueprintName) {
        ...
        if (structureBlueprint == null) {
            player.sendMessage(ChatColor.RED + blueprintName + " isn't a valid structure blueprint.");
            return;
        }
    ...
    public void onStructureBlueprintListNamedBlocks(Player player) {
        StructureBlueprintRecording structureBlueprintRecording = StructureBlueprintRecording.getBlueprintRecording(player);
        Set<Map.Entry<String, Vector>> namedBlocks = structureBlueprintRecording.getNamedBlocks();
        for (Map.Entry<String, Vector> namedBlockEntry : namedBlocks) {
    ...
    public void onStructureBlueprintLoad(Player player, String blueprintName) {
        try {
            ...
            StructureBlueprintRecording structureBlueprintRecording = structureBlueprint.toRecording();
            StructureBlueprintRecording.setBlueprintRecording(player, structureBlueprintRecording);
    ...
    @Subcommand("artillery gravity")
    public void onArtilleryGravity(Player player, double gravity) {
        MathUtils.FALLBLOCK_GRAVITY = gravity;
        player.sendMessage("Gravity: " + gravity);
    }

    @Subcommand("artillery select")
    public void onArtillerySelect(Player player) {
        Block targetBlock = player.getTargetBlock(5);
        if (targetBlock == null) {
            player.sendMessage(ChatColor.RED + "No block found");
            return;
        ...
        Artillery artillery = artilleryManager.getArtillery(targetBlock);
        if (artillery == null) {
            player.sendMessage(ChatColor.RED + "Artillery not found");
            return;
        }

        ArtilleryListener.SELECTED_ARTILLERY.put(player, artillery);
        player.sendMessage("Artillery selected: " + artillery);
    }

    @Subcommand("artillery bounds")
    public void onArtilleryBounds(Player player) {
        Block targetBlock = player.getTargetBlock(5);
        if (targetBlock == null) {
            player.sendMessage(ChatColor.RED + "No block found");
            return;
        ...
        player.sendMessage("MinX: " + artillery.qtXMin());
        player.sendMessage("MaxX: " + artillery.qtXMax());
        player.sendMessage("MinZ: " + artillery.qtZMin());
        player.sendMessage("MaxZ: " + artillery.qtZMax());
        player.sendMessage("MinY: " + artillery.getMinY());
        player.sendMessage("MaxY: " + artillery.getMaxY());
    }
